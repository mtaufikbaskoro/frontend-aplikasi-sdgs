...
export default function PemerintahDaerah () {
    ...
    const fetchDetailSubKegiatanById = async (subkegiatanId) => {
        setIsLoading(true)
        const res = await fetch(getUrl(`/api/realisasi/daerah/action?renjaSubkegiatanId=${subkegiatanId}`), {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        })
    ...
    useEffect(() => {
        const fetchProgramsInIndikatorsByKode = async (goal, page) => {
            ...
            const res = await fetch(getUrl(`/api/realisasi/daerah/subKegiatanByKode?sdgs_tujuan_kode=${goal}&pagination=${stringPagination}`), {
                method: 'GET',
                headers: {'Content-Type': 'application/json'}
            })
            if (res.ok) {
                const result = await res.json() 
                const { data, pagination, error } = result
                if (!error) {
                    setIndikators(data)
            ...
            <div className="flex flex-col gap-8">
                ...
                <div className={`${selectedGoal != null ? 'opacity-100' : 'opacity-0'} flex flex-col gap-6 transition-all ease-in ease-out`}>
                    <div className={`mx-3 px-3 py-2.5 border-2 border-slate-700 bg-slate-100 rounded-md drop-shadow-md`}>
                        <h1 className="capitalize text-md font-semibold">tujuan {selectedGoal?.kode} - {selectedGoal?.nama}</h1>
                    </div>
